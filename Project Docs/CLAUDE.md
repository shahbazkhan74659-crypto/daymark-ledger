...
- **Updated 2026-09-22 — the owner has directed that this project be marked complete as a development effort.** Every phase (0 through 19) is now `Status: Complete` — see `PHASES.md`'s top-of-file note for what is and isn't in scope of that completion (the automated test suite and `TASKS.md`'s remaining "Next" items are explicitly follow-up work, not gaps in the roadmap). Do not resume implementation work on this project on your own initiative going forward; any further work is a new effort requiring the owner's explicit direction.
- **Updated 2026-09-23 — development is resuming per the owner's explicit direction.** The owner's father (the actual end-user admin) has requested additional features. Development resumes as a continuation of the Phase 0–19 roadmap (they remain `Status: Complete`), not as a "separate new effort" — new feature work will be scoped as Phase 20+ via further owner consultation per rule 3. See `DECISIONS.md`'s "Development resumed — new feature requests" entry.
