...
**Implemented:** A local PostgreSQL 18 database (`daymark_ledger_dev`, see Phase 1 in `PHASES.md`), an Express 5 + TypeScript backend (`backend/`, see Phase 2 in `PHASES.md`) with a `GET /health` liveness endpoint, a `GET /api/db-check` endpoint (Phase 6) proving DB connectivity via a raw Prisma query, a real auth backend (Phase 7): `POST /api/auth/login`, `POST /api/auth/logout`, `GET /api/auth/me`, and session-verification middleware, backed by Prisma's first real migration (`User`/`Session` models), and the first domain models (Phase 9): `Worker`/`Attendance`, with `GET /api/workers` (active workers + today's status) and `POST /api/workers/:id/attendance/today` behind session auth. Prisma (CLI + Client, see Phases 3–4, 7, 9 in `PHASES.md`) is installed, connected to that database via a Postgres driver adapter, with its CLI tooling (`generate`/`validate`/`format`/`migrate dev`/`migrate status`/`studio`/`seed`) fully verified via `backend/package.json`'s `prisma:*` scripts. A React 19 + Vite 8 + TypeScript + Tailwind CSS v4 frontend (`frontend/`, see Phase 5 in `PHASES.md`) has a real, working login screen wired to the Phase 7 backend, `react-router-dom`-based routing, a Context-based auth state (Phase 8), and — as of Phase 10 — a real mobile-first home screen (worker list with inline today-status change, a floating quick-actions menu) wired to the Phase 9 API, matching the UI prototype's exact visual design. The full frontend → backend → database → backend → frontend round trip (Phase 6) is proven, gated behind real authentication end-to-end, and now serving real domain data. No Worker Detail content, Manage Employees, Create Employee, Reporting, or Settings screens exist yet (Phase 11+). The repository also contains this `Project Docs/` documentation system and a `Prototype/` folder (see "Prototype" below).
...
- **ORM (implemented, Phases 3–4, 7, 9):** Prisma 7.10.0 (CLI + `@prisma/client`, pinned to matching versions), connected to Postgres via `@prisma/adapter-pg` (this Prisma version requires an explicit driver adapter — no built-in engine-binary connection). Config lives in `prisma7.config.ts` (not `schema.prisma`'s `env()`, per this version's setup) and is auto-discovered by the CLI despite its non-default filename. `backend/package.json`'s `prisma:validate`/`prisma:format`/`prisma:generate`/`prisma:migrate:dev`/`prisma:migrate:status`/`prisma:studio`/`prisma:seed` npm scripts wrap the CLI (each pinned to `--config prisma7.config.ts` for self-documentation) and are all confirmed working end-to-end against `daymark_ledger_dev`. Schema has `User`/`Session` (Phase 7) plus the first domain models, `Worker`/`Attendance` (Phase 9, see "Data / Persistence" below).
...
`backend/` layout (as of Phase 9):
```text
backend/
  src/
    index.ts               — Express app bootstrap; GET /health (liveness), GET /api/db-check (Phase 6), mounts /api/auth (Phase 7) and /api/workers (Phase 9); express.json()/cookieParser() wired in
    ...
    lib/
      ...
      date.ts                 — todayDateOnly(): the server's local calendar date anchored at UTC midnight, for @db.Date columns (Phase 9)
    middleware/
    ...
    routes/
      ...
      workers.ts               — GET / (active workers + today's status), POST /:id/attendance/today (upsert, or clears the day's record when status: null) (Phase 9)
    generated/prisma/      — generated Prisma Client (gitignored, regenerated via `prisma generate`)
  prisma/
    schema.prisma          — User, Session, Worker, Attendance models
    seed.ts                 — seeds the admin user (ADMIN_USERNAME/ADMIN_PASSWORD) and 8 sample workers (Phase 9 — no create-worker endpoint exists yet)
    migrations/              — add_user_session, add_worker_attendance
...
**Auth and home-screen endpoints implemented (Phases 7, 9), other business endpoints still planned:** A REST API served by the Express backend, CORS-enabled for the frontend's origin (with `credentials: true` so the session cookie is sent). `GET /health` (Phase 2, liveness only), `GET /api/db-check` (Phase 6), `/api/auth/login` / `/api/auth/logout` / `/api/auth/me` (Phase 7), and `GET /api/workers` / `POST /api/workers/:id/attendance/today` (Phase 9) exist under this pattern; the `/api` prefix is intended for future business routes. Response envelope convention, established by the auth routes and followed by `workers.ts`: `{ status: "ok", ...data }` on success, `{ status: "error", message }` on failure — the frontend's `lib/api.ts` fetch wrapper parses this envelope generically. Remaining endpoints/resource shape not yet designed — implied by the data model: worker CRUD beyond today's status (create, personal-info update, Active/Inactive toggle), any-date attendance, advances (per worker, dated entries), plus whatever endpoints the auto-calculated totals need — all of which will require a valid admin session via the Phase 7 `requireSession` middleware. Not yet decided whether totals are computed server-side (returned pre-calculated) or client-side (raw records fetched, calculated in the frontend).

## Data / Persistence

**Connection implemented (Phase 3); User/Session schema (Phase 7) and the first domain schema, Worker/Attendance (Phase 9), implemented:** PostgreSQL 18 (`daymark_ledger_dev` locally), accessed via Prisma Client + `@prisma/adapter-pg`, confirmed connecting successfully. One central database is the single source of truth for all devices — this is the mechanism that satisfies the "open on any device" requirement (see `DECISIONS.md`). Phase 7's migration added `User` (bcrypt-hashed `passwordHash`) and `Session` (SHA-256-hashed `hashedToken`, `expiresAt`, cascading FK to `User`). Phase 9's migration (`add_worker_attendance`) added `Worker` (`fullName`, `designation`, `contact`, `joiningDate` as `@db.Date`, `perDayRate` as `Decimal(10,2)`, `status` as a `WorkerStatus` enum) and `Attendance` (`workerId` FK with cascade delete, `date` as `@db.Date`, `status` as an `AttendanceStatus` enum, `@@unique([workerId, date])` so each worker has at most one record per calendar day, `@@index([date])` for future cross-worker date queries). Per-worker data (profile, documents, photo, attendance-by-date, advance entries, salary rate) persists in the database and must survive a worker being marked Inactive and later reactivated — historical attendance/salary data must remain intact across that transition; no Worker delete/toggle endpoint exists yet to exercise this (Phase 15b). Advance entries and any-date attendance editing are not yet designed — start in Phase 11 per `PHASES.md`.
